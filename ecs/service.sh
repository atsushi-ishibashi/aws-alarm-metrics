...
aws cloudwatch put-metric-alarm \
  ...
  --alarm-description "1分間における、サービスのCPU使用率の平均が${CPU_THRESHOLD}%以上" \
  --ok-actions $ACTION \
...
aws cloudwatch put-metric-alarm \
  --alarm-name "awsecs-$CLUSTER-$SERVICE-MemoryUtilization" \
  ...
  --comparison-operator GreaterThanOrEqualToThreshold \
  --alarm-description "1分間における、サービスのメモリ使用率の平均が${MEM_THRESHOLD}%以上" \
  --ok-actions $ACTION \
  --alarm-actions $ACTION \
  --dimensions Name=ClusterName,Value=$CLUSTER Name=ServiceName,Value=$SERVICE \
  --profile $PROFILE \
